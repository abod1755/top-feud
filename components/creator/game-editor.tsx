...
export function GameEditor({ game, initialRounds }: { game: EditorGame; initialRounds: EditorRound[] }) {
  ...
  function save() {
    setError(null);
    startSave(async () => {
      const res = await saveGame(game.id, { title, tagline, description, difficulty }, rounds);
  ...
  function togglePublish() {
    ...
    startPublish(async () => {
      ...
      const saveRes = await saveGame(game.id, { title, tagline, description, difficulty }, rounds);
      if (!saveRes.ok) {
